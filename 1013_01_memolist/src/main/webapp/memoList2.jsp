...
//	없으면 0을 ResultSet 객체에 저장하므로 ResultSet 객체에 저장되는 데이터가 무조건 있기 때문에
//	물어보는 동작을 할 필요가 없다.
...
//	이전 페이지에서 currentPage가 넘어오지 않으면 넘어오는 값이 없으므로 currentPage는 null 이다.
//	null에 parseInt() 메소드를 실행하면 NumberFormatException이 발생되기 때문에 현재 페이지에 표시할
//	페이지 번호(currentPage)를 받을 때 예외 처리를 해야한다.
//	currentPage가 정상적으로 넘어오면 넘어온 currentPage를 정수로 바꿔서 currentPage변수에 저장하고
...
//	startNo = (totalCount - ((currentPage-1) * 10)); 
	startNo = (currentPage-1) * pageSize; 
//	out.println("현재 화면에 표시되는 글 목록의 시작 인덱스 번호: " + startNo + "번<br/>");	
...
//	한 페이지에 표시할 마지막 글의 인덱스는 전체 글의 개수보다 커지만 안되므로 마지막 글의 인덱스 번호가
...
//	5.=========================================================================================================
//	memolist.jsp에서 사용했던 select sql 명령을 수정해서 전체 글을 모두 얻어오던 것을 limit를 사용해서  
