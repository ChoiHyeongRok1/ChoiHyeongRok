...
//	페이지 이동 버튼의 시작 페이지 번호와 마지막 페이지 번호를 계산한다.
	...
	endPage = startPage + 9;
//	페이지 이동 버튼의 마지막 페이지 번호가 전체 페이지 수보다 커지면 존재하지 않는 페이지 번호가
//	표시되므로 마지막 페이지 번호를 전체 페이지 번호로 수정한다.
...
//	currentPage가 1보다 크다면 처음으로 이동할 수 있다.
	if (currentPage > 1){
%>
			<button type="button" onclick="location.href='?currentPage=1'" title="첫페이지로 이동합니다.">처음</button>	
<%		
	}else{
%>
			<button type="button" disabled="disabled" title="이미 첫페이지입니다.">처음</button>
<%		
	}
...
//	startPage가 1보다 크다면 이전 10페이지로 이동할 수 있다.
	if (startPage > 1){
%>
			<button type="button" onclick="location.href='?currentPage=<%=startPage-1%>'" title="이전페이지로 이동합니다.">이전</button>	
<%		
	}else{
%>
			<button type="button" disabled="disabled" title="이미 첫페이지입니다.">이전</button>
<%		
	}
...
%>
			<button type="button" onclick="location.href='?currentPage=<%=endPage+1%>'" title="다음페이지로 이동합니다.">다음</button>	
<%		
	}else{
%>
			<button type="button" disabled="disabled" title="이미 마지막 페이지입니다.">다음</button>
<%		
	}
...
%>
			<button type="button" onclick="location.href='?currentPage=<%=totalPage%>'" title="마지막 페이지로 이동합니다.">마지막</button>	
<%		
	}else{
%>
