...
//	2.
//	$().ready(function(){alert('jQuery 온로드2')})
	
//	3.
//	$().ready(() => {alert('jQuery 온로드3')})
...
//	아래와 같이 1줄에 여러개 스타일 속성을 지정할 수 있다.
//	$('img').css('height', '200px').css('width', '100px');
...
function highLight(){
//	document.getElementById('list').style.backgroundColor = 'yellowgreen'
//	$('#list').css('backgroundColor', 'skyblue');
//	$('#list').css('background-color', 'hotpink');
	$('#list').css('background-color', 'yellow');
//	document.getElementsByTagName('li')[1].style.backgroundColor = 'yellowgreen'
	$('#list > li').first().css('background-color', 'red');
	$('#list > li').last().css('background-color', 'yellowgreen');
	$('#list > li').eq(5).css('background-color', 'skyblue');
	$('#list > li').slice(2,4).css('background-color', 'hotpink');
...
//	jQuery는 전체작업을 할 때 반복문을 사용하지 않아도 된다.
