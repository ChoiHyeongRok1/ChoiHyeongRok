...
//	submit(html이나 javascript는 onsubmit) 이벤트는 input태그나 button태그의 type속성이
...
//	submit 이벤트의 리턴값이 true면 action페이지로 form의 데이터를 가지고 이동하고 false면
//	현재 페이지를 그대로 표시한다.
...
//	전체선택 체크 박스가 클릭되면 모든 체크 박스를 선택 또는 해제한다.
	$('input:checkbox[name=all]').click(() => {
		...
		// console.log(checked);
		// name 속성이 chk인 체크박스의 프로퍼티를 name 속성이 all인 체크박스 프로퍼티로 바꾼다.
...
//	모두 체크되었으면 전체선택 체크 박스를 체크하고 그렇지 않으면 체크안되게 한다.
...
//				console.log(chk.next().html());
				...
				$('#result').append('과일 이름: ' + chk.next().text() + ', 과일 가격: ' + chk.val() + '<br/>');
			})
		}
	})
