...
//	         private처럼 사용된다.
//	public: 현재 클래스 내부, 외보 어디에서나 자유롭게 접근할 수 있다.
...
//	년, 월 인수로 넘겨받아 그 달의 마지막 날짜를 리턴하는 메소드
...
//	static은 자바 프로그램이 실행하기 전에 static으로 선언된 함수(메소드)나 변수를 첫 단계로 메모리에 올려준다.
//	main 메소드가 실행되기 위해서는 메모리에 적재되어야 한다.
//	main 메소드가 메모리에 적재되어있지 않으면 프로그램의 시작점인 main() 메소드를 호출하려는데
//	메모리에 main() 메소드가 없으므로 실행을 할 수 없다.
//	그래서 main() 메소드는 누군가(자바 가상머신)가 호출하기 전에 자바 가상머신이 메모리에 적재시킨다.
...
//		날짜를 다 출력하고 남은 빈 칸에 다음달 날짜를 1일의 요일부터 토요일까지 반복하며 출력한다.
//		if(month == 12) {
