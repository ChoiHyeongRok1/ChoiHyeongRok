...
//		데이터 입력 단계에서 불필요한 공백을 제거하고 입력받으려면 아래와 같이 trim() 메소드를 사용한다.
...
//		lastIndexOf(): 메소드는 문자열의 오른쪽부터 검색해서 인수로 지정한 문자열이 최초로 나타나는 index를 얻어온다.
...
//		indexOf(), lastIndexOf() 메소드는 인수로 지정한 문자열이 없으면 -1을 리턴한다.
		...
		System.out.println(str.indexOf("It"));
		System.out.println(str.lastIndexOf("iT"));
//		indexOf(), lastIndexOf() 메소드의 실행결과가 0이상이면 인수로 지정된 문자열이 포함되었다는 의미로 사용할 수 있다.
		
		str = "itTjoeunit";
//		contains() 메소드는 문자열에 인수로 지정한 문자열이 포함되어 있으면 true, 없으면 false를 리턴한다.
...
//		replace(a, b) : 문자열의 모든 a를 b로 치환한다.
		...
		str = "123-4566546-12345";
		System.out.println("앞쪽 '-' index: " + str.indexOf('-'));
		System.out.println("앞쪽 '-' index: " + str.lastIndexOf('-'));
