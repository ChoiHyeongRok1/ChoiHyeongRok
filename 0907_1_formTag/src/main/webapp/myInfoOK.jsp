...
<body>
	...
	//  한글 깨짐을 방지하려면 최초의 request.getParameter() 메소드가 실행되기 전(맨 처음)에
	//  아래 내용을 코딩하면 된다.
	request.setCharacterEncoding("UTF-8");

	//  get 방식도 한글이 깨졌었다. => tomcat server 7.0부터 get 방식의 한글이 깨지지 않는다.
	//  get 방식에서 한글이 깨지면 서버 환경 설정에서 한글이 깨지지 않도록 설정해야 한다.
	//  Server 폴더의 server.xml 파일을 열고 Connector 태그에 URLEncoding="UTF-8"을 추가한다.
	//  서버에 추가하기 때문에 프로젝트를 생성할 때 마다 설정할 필요가 없고 이클립스를 다시 세팅할 때
	//  하면된다.
	...
	//  checkbox는 여러 항목을 선택할 수 있는데 아래와같이 request.getParameter() 메소드를 이용해
	//  넘어오는 데이터를 받으면 맨 처음 선택한 값 1개만 받을 수 있다.
	//  데이터가 여러개 넘어오는 경우 request.getParameter() 메소드를 사용하면 안된다.
	//    String hobbies = request.getParameter("hobbies");
	//    out.println(name + "님의 취미는 " + hobbies + "입니다.<br>");

	//  checkbox에서 넘어오는 데이터를 받을 때 넘어오는 항목이 여러개일 수 있기 때문에
...
//  태그사용 불가능, 줄바꿈 가능
    out.println(name + "님이 남긴 글: <br>" + content.replace("<", "&lt;").replace(">", "&gt;").replace("\r\n", "<br>") + "<br>");
