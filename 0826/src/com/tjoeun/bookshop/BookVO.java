...
// VO클래스는 DTO(Data Transfer Object)블래스라고도 부른다.
...
//	초기화를 하지 않아도 숫자는 0, 문자는 공백, boolean은 false, 클래스로 만드는 객체(참조 변수)는 null로
//	자동 초기화된다.
...
//	자용자가 생성자를 선언하면 기본 생성자를 자동으로 만들어 주지 않는다.
...
//		인수로 넘어온 writeDate에 저장된 날짜 데이터에서 년은 1900을 월은 1을 빼서 다시 지정한다.
...
//	@Override 어노테이션 이 메소드가 부모 클래스에서 상속받은 @Override한 메소드임을 의미한다.
...
//		NumberFormat 클래스는 singleton 패턴으로 제작되어있어서 객체를 생성하지 못하고 제작되어있는 서식을
//		얻어오는 메소드를 사용해서 클래스 내부에 미리 작성되어있는 서식을 얻어와 사용한다.

//		NumberFormat nf = NumberFormat.getNumberInstance(); // 천 단위 구분 기호
//		NumberFormat nf = NumberFormat.getCurrencyInstance(); //통화표시와 천단위 구분 기호
//		NumberFormat nf = NumberFormat.getPercentInstance(); // 백분율표시, 자동으로 100이 곱해진다.

//		DecimalFormat 클래스로 숫자 출력 서식 지정하기 => 만들어 사용한다.
//		DecimalFormat df = new DecimalFormat("출력서식");
//		출력서식에는 "#,##0"을 입력하고 앞, 뒤로 서식에 필요한 문자를 직접 입력해서 만든다.
//		소숫점 아래 자리수는 "."을 입력하고 보고싶은 만큼 0을 쓰면된다
