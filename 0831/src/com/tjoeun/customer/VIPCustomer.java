...
//  우수 고객 정보를 기억하는 클래스
//  이미 Customer 클래스에서 구현한 내용이 중복되므로 Customer 클래스를 상속(확장)받아 만든다.
...
//		customerGrade = "SILVER"; //에러
//		bonusRatio = 0.01; //에러
//		private 권한의 필드는 접근할 수 없으므로 setter를 실행해서 VIP고객 정보를 넣어준다.
		setCutomerGrade("VIP");
		setBonusRatio(0.05);
		salesRatio = 0.1;
	}
...
//	Customer 클래스에서 상속받은 calBonus() 메소드는 구매 금액에 대한 보너스 포인트를 계산하기 때문에
//	VIP 고객의 할인율이 적용된 금액에 대한 보너스 포인트는 계산할 수 없다.
//	Customer 클래스에서 상속받은 calBonus() 메소드를 Override 해서 구현한다.
	
//	구매 금액을 넘겨받아 실 구매 금액에 따른 보너스 포인트를 계산하는 메소드
	@Override
	public int calBonus(int price) {
		
		return (int)(price * (1 - salesRatio)*getBonusRatio());	
