...
//		super();  // 부모클래스의 기본생성자, 코딩하지 않더라도 자바 컴파일러가 자동으로 만들어준다.
//		this()와 super()는 반드시 생성자의 첫 문장으로 써야한다. => 가장 먼저 실행된다.
//		부모 클래스의 생성자가 자식 클래스의 생성자보다 먼저 실행된다.
//		부모 클래스의 기본 생성자를 정의하지 않으면 자식 클래스에서 super()를 실행할때 에러가 발생된다.
		System.out.println("자식 클래스의 생성자 실행");
	}
//	이클립스가 지원하는 생성자 자동 완성 기능을 사용할 때 현재 클래스에 물리적으로 정의하지 않은 필드
//	(상속받은 필드)는 생성자를 만드는 대화상자에 표시되지 않는다.
//	부모 클래스에서 상속받은 private권한의 필드를 초기화하는 방법은 부모 클래스에서 상속받은 필드에 
//	접근 권한이 private일 경우 자식 클래스에서 접근할 수 없으므로 부모 클래스의 생성자를 호출해서 초기화시킨다.
...
//	}
//	부모 클래스에서 상속받은 필드의 접근 권한이 protected일 경우 자식 클래스에서도 접근할 수 있으므로
//	this를 하용하는 초기화가 가능하다.
	public Child(String name, boolean gender, int age, String nickname) {
	    this.name = name;
	    this.gender = gender;
		this.age = age;
...
//	부모 클래스에 private 권한으로 설정된 모든 필드의 setter가 정의되어 있다면 setter의
//	접근권한은 메소드이므로 public을 사용하고 public 권한은 자식클래스에서 실행할 수 있으므로
//	부모 클래스에서 상속받은 private권한의 필드를 setter를 이용해서 초기화시킬 수 있다.
//	@Override(재정의): 부모 클래스로부터 상송받은 메소드의 기능을 무시하고 다시 만들어 사용한다.
	@Override 
	public String toString() {
//		부모 클래스에서 상속받은 private 권한을 가지는 필드는 자식에서 접근할 수 없으므로
//		부모 클래스에서 상속받은 private 권한을 가지는 필드에 저장된 데이터는 getter 메소드를
//		사용해서 접근해야 한다.
//		return타입이 boolean인 메소드는 "is"로 메소드 이름을 시작하는게 관행이다.
		
//		자식 클래스에서 부모 클레스에서 상속받은 메소드의 앞, 뒤에 기능을 추가할 수 있다.
