...
//	같은 이름으로 변수를 다시 선언했음에도 불구하고 에러가 발생되지 않고 각기 다른 값이 출력되는 것을 볼 수 있다.
var name = 'javascript';
console.log('name: ' + name);

//	이 방식은 유연한 변수 선언으로 간단한 테스트에는 편리할 수 있으나 코드의 양이 많아지면 어디에서 어떻게 사용될지
//	파악하기 힘들고 변수에 저장된 값이 변경될 우려가 있다.
//	이를 보완하기 위해서 추가된 변수 선언 방식이 let과 const이다.

let id = 'lim';
console.log('id: ' + id);
//	let으로 선언한 id라는 변수를 같은 이름으로 다시 선언하면 이미 변수가 선언된 상태이므로 에러가 발생
id = 'jang';
console.log('id: ' + id);

const test = 'test' // 상수 선언
console.log('test: ' + test);
//	let뿐만 아니라 const를 사용해서 변수를 선언해도 같은 변수를 선언하면 에러발생
//	const는 상수선언 이므로 상수에 값을 할당하면 에러발생
...
//	지역 변수: 함수 내부에서 선언한 변수 선언된 함수 내부(var) 또는 {}블록 내부(let, const)에서만 변수에 저장된 데이터를 사용할 수 있다.
//			   전역변수과 같은 이름의 지역변수를 사용해야 한다면 지역 변수를 선언할 때 반드시 var를 붙여서 선언해야 한다.
...
//	함수 내부에서 아래와 같이 var를 이용해서 전역 변수과 같은 이름의 지역 변수를 선언하면 함수 내부의 variable 이라는
//	이름을 가지는 지역 변수로 선언되지만 var를 사용하지 않으면 전역 변수를 의미한다.
...
function test3(){
	...
	document.getElementById('var2').innerHTML = '<b style="color: green; background-color: yellow;">' +  variable2 + '</b>'
}
