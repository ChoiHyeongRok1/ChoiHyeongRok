...
function strTest3(){
	let str = prompt('what is your name?', "Do not type number");
...
//	'==', '!='는 데이터 타입은 비교하지 않고 데이터만 비교한다. '1'과 1은 같은 값으로 취급한다.
//	'===', '!=='는 데이터 타입과 데이터를 모두 비교한다. '1'과 1은 다른 값으로 취급한다.
	let num = 10;
...
function strTest4(){
//	indexOf(): 문자열 왼(앞)쪽부터 인수로 지정한 문자열을 찾아서 index를 리턴한다.
//	lastIndexOf(): 문자열 오른(뒤)쪽부터 인수로 지정한 문자열을 찾아서 index를 리턴한다.
//	indexOf(), lastIndexOf() 모두 인수로 지정한 문자열이 없으면 -1을 리턴한다.
	...
	if (name.indexOf(str)==-1){
		...
		console.log('indexOf() 함수로 검색한 index: ' + name.lastIndexOf(str))
	}
}
