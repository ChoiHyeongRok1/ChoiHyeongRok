superheroes = ['아이언맨', '캡틴아메리카', '토르', '닥터스트레인지'];
...
//	map(): 배열의 요소들을 대상으로 특정 작업을 실행 결과를 배열로 리턴한다.
...
//	indexOf(): 배열 전체에서 인수로 지정한 첫번째 요소를 찾아 index를 리턴한다. 없으면 -1을 리턴한다.
const index = superheroes.indexOf('캡틴아메리카');
console.log(index);
if(index >= 0){
console.log('있음');
} else{
...
//	findIndex(): 배열에서 인수로 지정된 조건을 만족하는 첫번째 요소의 index를 리턴한다. 없으면 -1을 리턴한다.
...
//	find(): 배열에서 인수로 지정된 조건을 만족하는 첫번째 요소를 리턴한다.없으면 undefinded를 리턴한다.
const index3 = todos.find(function(todo){
...
const index4 = todos.filter(function(todo){
