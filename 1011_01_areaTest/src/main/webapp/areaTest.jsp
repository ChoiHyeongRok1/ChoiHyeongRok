...
//	영역 변수에 저장된 데이터는 영역이름.getAttribute("영역 변수 이름") 과 같이 얻어온다.
...
//	저장된 데이터는 저장할 데이터 타입으로 반드시 형변환시켜서 사용해야 한다.
	...
	out.println("pageContextVar 영역 변수: " + pageContextVar + "</br>");
	
//	sendRadirect(): 인수로 지정된 페이지로 넘겨준다.
//	sendRadirect() 메소드는 request 영역의 데이터를 가지지 않고 인수로 지정된 페이지로 넘어간다.
//	sendRadirect() 메소드는 주소창에 표시되는 페이지 이름이 인수로 지정된 페이지 이름으로 변경된다.
//	현재 페이지와 sendRadirect() 메소드의 인수로 지정한 페이지가 완전히 분리된다.
...
//	현재 페이지와 forward() 메소드의 인수로 지정한 페이지가 연결이 된 상태이다. => 새로고침하면 주소창에 표시된 
//	forword() 메소드가 입력된 페이지부터 현재 화면에 보이는 페이지까지 실행된다.
