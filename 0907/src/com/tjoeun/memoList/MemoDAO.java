...
//	테이블에 저장된 메모 목록을 최신글 순으로 얻어오는 메소드
	public static ArrayList<MemoVO> select() {
		...
		try {
			...
			pstmt = conn.prepareStatement(sql);
//			select spl 명령의 실행 결과를 ResultSet 객체에 저장한다.
			rs = pstmt.executeQuery();
//			테이블에서 성공적으로 데이터를 얻어왔으므로 결과를 저장해서 리턴키실 ArrayList
//			객체를 만든다.
			list = new ArrayList<>();

//			ResultSet 객체에 데이터가 있는 동안 반복하여 ResultSet 객체에 저장된 데이터를
//			MemoVO객체로 만들어서 ArrayList에 저장한다.
...
//				MemoVO 클래스 객체를 ArrayList에 저장한다.
	...
	public static MemoVO selectByIdx(int idx) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
//			테이블에서 글 1건을 얻어왔으므로 결과를 저장해서 리턴시킬 MemoVO클래스 객체를 만든다.
				vo = new MemoVO();
				vo.setIdx(rs.getInt("idx"));
				vo.setName(rs.getString("name"));
				vo.setPassword(rs.getString("password"));
