...
//  	month 13이 넘어왔으면 달력에는 다음해 1월을 표시해야하고 month에 0이 넘어왔으면 달력에는 전년도 12월을 표시해야 한다.
		if(month == 13){
			year++;
			month = 1;
		}else if(month == 0){
	...
	<tr>
		<th>
			<!-- 
				...
				href 속성에 "#"뒤에 id(해시)를 지정하면 현재 문서에서 id가 지정된 요소로 이동(책갈피)
		...
		<th>
			<%-- <a href = "?year=<%=year%>&month=<%=month + 1%>">→</a> --%>
			<button class="button button1" type="button" onclick="location.href='?year=<%=year%>&month=<%=month + 1%>'">다음달</button>
		</th>
	</tr>
