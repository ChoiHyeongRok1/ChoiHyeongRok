...
<%
//	쿠키를 읽어올때는 한번에 읽어와야 하기 때문에 배열로 받는다.
...
//	out.println(Arrays.toString(cookies) + "<br/>");
	for (Cookie cookie : cookies){
		...
		out.println("쿠키 이름: " + cookie.getName() + ", 쿠키내용: " + cookie.getValue() + "<br/>");
		
		// 8.0 이하 버전에서는 내용이 한글인 쿠키를 저장할 때 에러가 발생되기 때문에 인코딩 과정을 거쳐서
		// 쿠키를 저장해야 했지만 읽어올때는 그냥 읽어와도 에러가 발생되지 않는다.
		// 다만, "%EC%98%84~~~"와 같이 인코딩된 유니코드 자체를 가져오기 때문에 읽을 수 없다.
		// 8.0 이하 버전에서 인코딩되서 저장된 쿠키 내용을 읽으려면 디코딩 과정을 거쳐야 읽을 수 있다.
		// 8.5 버전도 한글이 %EC%98%84~~~와 같이 보이므로 디코딩 과정을 거쳐야 한다.
