...
<%-- 	
	...
	위의 2줄을 EL을 사용하면 ${guestbookList} 과 같이 나타낼 수 있다.
						...
						<table class="table" width="1000" align="center" border="1" cellpadding="5" cellspacing="0">
							<tr>
								<td>
									...
									type="button" 
									value="삭제" 
									onclick="location.href='selectByIdx.jsp?idx=${vo.idx}&currentPage=${guestbookList.currentPage}&job=delete'"><br/>
		...
		<tr>
			<td align="center">
				...
				<c:if test="${guestbookList.startPage > 1}">
					<button class="button" type="button" onclick="location.href='?currentPage=${guestbookList.startPage - 1}'" title="이전페이지로 이동합니다.">이전</button>
				</c:if>
				<c:if test="${guestbookList.startPage <= 1}">
				...
				<c:if test="${guestbookList.endPage < guestbookList.totalPage}">
					<button class="button" type="button" onclick="location.href='?currentPage=${guestbookList.endPage+1}'" title="다음페이지로 이동합니다.">다음</button>
				</c:if>
				<c:if test="${guestbookList.endPage >= guestbookList.totalPage}">
				...
				<c:if test="${guestbookList.currentPage < guestbookList.totalPage}">
					<button class="button" type="button" onclick="location.href='?currentPage=${guestbookList.totalPage}'"title="마지막 페이지로 이동합니다.">마지막</button>	
				</c:if>
				<c:if test="${guestbookList.currentPage >= guestbookList.totalPage}">
