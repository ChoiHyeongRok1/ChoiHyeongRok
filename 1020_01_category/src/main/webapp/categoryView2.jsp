...
<%@ taglib prefix="rn" uri="http://java.sun.com/jsp/jstl/functions"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body class="p-3">
	...
	<c:forEach var="vo" items="${categoryList.list}">
		...
		<form id="form" class="sub_form row m-1" action="reply.jsp" method="post" name="${formName}">
			...
			<div class="col-md-5">
				<c:if test="${vo.deleteReport < 10}">
					<input class="btn btn-outline-info" type="submit" value="서브 카테고리 만들기">
					<c:if test="${vo.deleteCheck == 'YES'}">
						<input class="btn btn-danger" type="button" value="삭제" disabled="disabled">	
						<%-- <input class="btn btn-outline-warning" type="button" value="복구" onclick="location.href='restore.jsp?idx=${vo.idx}'"> --%>
						<input class="btn btn-outline-warning" type="button" value="복구" onclick="mySubmitRestore(${formName})">
					</c:if>
					<c:if test="${vo.deleteCheck != 'YES'}">
						...
						<input class="btn btn-warning" type="button" value="복구" disabled="disabled">
					</c:if>
