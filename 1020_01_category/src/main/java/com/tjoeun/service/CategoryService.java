...
//	insert.jsp에서 호출되는 테이블에 저장할 메인 카테고리 정보가 저장된 객채를 넘겨받고 mapper를 얻어온 후
...
//		실행한 sql명령이 테이블을 변경하는 insert, delete, update sql 명령일 경우 작업 결과를 테이블에
//		반영시키기 위해 작업이 완료되면 commit()메소드를 실행해야 한다.
//		테이블이 변경되지 않는 select sql명령은 사용할필요 없다.
		mapper.commit(); // 선택적
...
//	reply.jsp에서 호출되는 테이블에 저장할 서브 카테고리 정보가 저장된 객채를 넘겨받고 mapper를 얻어온 후
...
//	delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 클래스의
//	카테고리 1건을 얻어오는 delete sql 명령을 실행하는 메소드를 호출하는 메소드
	...
	public void deleteUpdate(int idx) {
		System.out.println("CategoryService의 deleteUpdate() 메소드");
		SqlSession mapper = MySession.getSession();
		CategoryDAO.getInstance().deleteUpdate(mapper, idx);
		mapper.commit();
		mapper.close();
	}
	...
	public void restoreCheck(int idx) {
		System.out.println("CategoryService의 restoreCheck() 메소드");
		SqlSession mapper = MySession.getSession();
