...
public class MemoList {
	...
	public String toString() {
		String str = "";
		if(memoList.size() == 0) {//memoList에 저장된 클이 없는가?
...
//	MemoMain 클래스에서 호출되는 memoList라는 ArrayList에 저장할 데이터가 저장된 MemoVO 클래스 객체를
...
//	MemoMain 클래스에서 호출되는 수정 또는 삭제할 글번호를 넘겨받고 memoList라는 ArrayList에 저장된
//	메모 중에서 수정 또는 삭제할 글번호에 해당되는 글 1건을 얻어와서 리턴하는 메소드
...
//		글 삭제 후 인덱스가 변경되므로 인덱스에 맞춰서 글번호를 다시 붙여준다.
//		이 현상은 데이터베이스를 사용하지 않고 ArrayList를 사용하기 때문에 발생되는 현상이다.
//		ArrayList는 앞의 데이터가 지워지면 뒤의 데이터가 앞으로 인데스가 당겨지는 특징이 있다.
