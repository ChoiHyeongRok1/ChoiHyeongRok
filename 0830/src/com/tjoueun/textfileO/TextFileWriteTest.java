...
//	파일의 경로와 이름을 설정한다.
//	경로 지정방식은 절대결로 지정방식과 상대경로 지정방식이 있다.
...
//	결로 지정시 경로와 경로, 경로와 파일을 구분하는 "\"가 에러가 발생되면 "\\"나 "/"로 변경한다.

//	절대경로
//		String filePath = "D:\\cccc\\java\\work\\data\\data\\out.txt";
//	String filePath = "D:/cccc/java/work/data\\out.txt";
...
//				보이지 않는다면 "\r\n"을 사용한다.
//				\n: new line, 줄을 바꾼다.
//				\r: carrage return, 커서를 그 줄의 맨 앞으로 보낸다.
				if (str.length() != 0) {
...
//			e.printStackTrace();
			...
			if (printWriter != null) {
//				출력파일은 출력 작업이 완료되면 반드시 파일을 닫아야 정상적으로 파일이 생성된다.
				printWriter.close();
			}
		}
	}
