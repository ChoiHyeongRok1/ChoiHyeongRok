...
//		부모 클래스 타입으로 부모 클래스 타입의 객체를 만들어 상용하면 아무 문제없이 처리된다.
		Base base = new Base();
		base.name = "홍길동";
		base.say();
		System.out.println("===================================");
		
//		자식 클래스 타입으로 자식 클래스 타입의 객체를 만들어 상용하면 아무 문제없이 처리된다.
		Sub sub = new Sub();
		sub.name = "임꺽정";
		sub.age = 20;
		sub.say();
...
//		부모 클래스 타입에 자식클래스 타입의 객체가 생성된 주소를 대입하면 아무런 문제없이 처리된다.
...
//		downcating은 부모 클래스 타입으로 upcasting된 자식 클래스를 다시 자식 클래스 타입으로 변환하는것을 말한다.
		Base b = sub;
//		b는 부모 클래스 타입의 객체지만 자식 클래스 타입의 객체가 생성된 주소를 대입했으므로
//		say() 메소드를 실행하면 부모 클래스의 say() 메소드가 아닌 자식클래스의 say()메소드가 실행된다.
		b.say();
		System.out.println("===================================");
...
//		casting 시켜서 대입하면 대입은 가능하다. 이 경우 문법적인 오류는 발생되지 않지만 프로그램 실행 단계에서
//		ClassCastException이 발생된다.
	
//		Sub s = (Sub) base; //casting 시키면 문법적인 에러는 발생되지 않는다.
		
//		instanceof 연산자는 객체가 instanceof 뒤에 저장한 클래스 타입으로 안전하게 형변환이 가능한지 검사한다.
